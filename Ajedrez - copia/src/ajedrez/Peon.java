...
public class Peon extends Ficha {
    ...
    public void moverPeon(Ficha[][]tablero) {

        System.out.println("donde?");
        int dondex = sc.nextInt();
        int dondey = sc.nextInt();

        if ((Math.abs(posx - dondex)) == 1) {//movimiento normal
            Ficha ficha = tablero[dondex][dondey];
            switch (Math.abs(posy - dondey)) {
                case 0:
                    //delante y que no pueda ir atras
                    if (dondey<posy&&ficha.getNombre().equals("_")) {
                        posx = dondex;
                        posy = dondey;
        ...
        } else if ((Math.abs(posx - dondex)) == 2) {//primer movimiento
            if (primeraVez) {
                Ficha ficha = tablero[dondex][dondey];
                switch (Math.abs(posy - dondey)) {
                    case 0:
